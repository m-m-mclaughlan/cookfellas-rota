window.applyWlbOptimizerPatch = function (html) {
  try {
    ...
    // Manager coverage is site-wide and based on physical presence at the
    // earliest site start / latest site finish, not on which ticket happens
    // to carry the OPEN/CLOSE label.
    ...
    const fn = `,CtWlbOpt=(e,t,n,a,l,h=2)=>{
      let y={...n},x={...l},R=Object.fromEntries(e.map(s=>[s.id,s])),
          ...
          isWlb=t.some(s=>s.day==="Mon"&&s.time==="11:30-9")&&t.some(s=>s.day==="Sat"&&s.time==="11-9")&&a.some(s=>s.day==="Sun"&&s.time==="11-6");
      ...
      // Remove the old auto-generated manager pattern first. This is what
      // prevents four-day split-shift runs from surviving into the WLB rota.
      ...
      // Mark: Sun/Mon off, Tue-Sat. 42.5h on the current WLB template.
      anchor("Mark","r","Tue","11-9","floor");
      anchor("Mark","r","Wed","11:30-9","floor");
      anchor("Mark","b","Thu","2-f","bar");
      anchor("Mark","r","Fri","6-f","floor");
      anchor("Mark","r","Sat","11-9","floor");

      // Fran: Wed/Thu off, restaurant-led Fri-Tue pattern.
      ...
      // Tyler: Wed/Thu off, continuous Bar core/close shifts rather than
      // 11:30-2:30 + 5-f splits. Total 41h on this template.
      anchor("Tyler","b","Mon","2-f","bar");
      anchor("Tyler","b","Tue","2-f","bar");
      anchor("Tyler","b","Fri","2-f","bar");
      anchor("Tyler","b","Sat","2-f","bar");
      anchor("Tyler","b","Sun","11-6","bar");

      // George bridges the Wed/Thu management gap and supports Friday close.
      anchor("George","r","Wed","5-f","floor");
      anchor("George","r","Thu","11:30-9","floor");
      anchor("George","r","Fri","5-f","floor");

      // Remove any overlaps left by the earlier scheduling passes. Anchors win;
      // otherwise keep the longer continuous assignment.
      ...
      let coreIds=new Set(core.map(s=>s.id)),
          candidateScore=(s,q)=>{let def=s.contractType==="contracted"?Math.max(0,s.contractedHours*60-mins(s.id)):0,area=q.board==="b"?(s.isBarStaff?-500:500):(s.isBarStaff?500:-500),mgr=s.isManager?250:0;return-def*100+area+mgr+mins(s.id)/10},
      ...
      // Fill every gap created when the old manager pattern was removed.
      ...
      // Hard final manager coverage using actual presence at site open/close.
      for(let day of m)for(let when of["open","close"]){
        ...
        for(let q of targets)for(let s of e.filter(s=>s.isManager)){
          ...
          // Temporarily ignore the current occupant of the target while testing overlap.
      ...
      // If one of the three principal managers was available Saturday but an
      // anchor could not be used, still give them a non-overlapping Saturday
